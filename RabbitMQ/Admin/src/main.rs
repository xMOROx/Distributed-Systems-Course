...
async fn init() -> Init {
    ...
    let names_of_queues: Vec<String> = vec!["log".to_string(), INFO_EXCHANGE.to_string().to_lowercase()];


    server.declare_exchange(INFO_EXCHANGE, ExchangeType::Direct)
        .await.expect("Declare exchange failed");

    server.bind_to_queues_exchange(INFO_EXCHANGE, names_of_queues.clone())
